...
class CheckCashExistView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        elif (timedelta(days=1) - (datetime.today() - current_cash_obj.created_date_time)).days:
            unsettled_invoices = InvoiceSales.objects.filter(is_settled=0, cash_desk=current_cash_obj,
                                                             is_deleted=0).count()
            if unsettled_invoices:
                return JsonResponse(
                    {"response_code": 3, 'error_msg': OLD_CASH_WITH_UNSETTLED_INVOICES,
                     'error_mode': 'OLD_CASH_WITH_UNSETTLED_INVOICES'})
            else:
...
class InvoiceSalesByCashView(View):
    ...
    def get(self, request, cash_id, *args, **kwargs):
        ...
        return JsonResponse({
            ...
            'created_date_time': jdatetime.datetime.fromgregorian(datetime=current_cash.created_date_time).strftime(
                "%H:%M %Y/%m/%d") if
            current_cash.created_date_time else '',
            'ended_date_time': jdatetime.datetime.fromgregorian(datetime=current_cash.ended_date_time).strftime(
                "%H:%M %Y/%m/%d") if
